...
let ratio = mapX / mapY;
canvas.width = (window.innerHeight - canvas.offsetTop) * 0.99;
...
export const drawAll = function (td = false) {
...
export const drawRay2D = function (ax, ay, bx, by, colour) {
...
export const drawRay3D = function (x, distance, lineWidth, colour) {
    let lineHeight = (mapS * ctx.canvas.height) / distance;
    if (lineHeight > ctx.canvas.height)
        lineHeight = ctx.canvas.height;
